    ...
    <div class="container">
        <div class="row">
            <div class="col-md-10" style="padding: 10px; margin: auto; background-color: white; margin-bottom: 20px; margin-top: 30px; border-radius: 15px;">
                ...
                @if($c->id == $forum->customer_id)
                <div class="ten">
                <img src="{{url('avatar')}}/{{$c->avatar}}" alt="" style="width: 45px; height: 45px; border-radius: 100%;">

                <div>
                    <span style="display: none;">{{$date=$forum->created_at}}{{$n= strtotime($date)}}</span>
               ...
               @foreach($customer as $c)
                ...
                <div style="background-image:linear-gradient(120deg, #3ca7ee, #9b408f); margin-top: 5px; display:flex; justify-content: center; align-items:center ;font-family: monospace;font-size: 18px; color: white;">
                    <div>
                ...
                <div style="width: 100%; display: flex; border-top: 1px solid green;border-bottom: 1px solid green; margin-top: 1em; margin-bottom: 1em;">
                    <table class="bang">
                        <tr>
                            <td>Hãy văn minh</td>
                            <td style="text-align: right;">{{$dem1 >0 ? 'Có '.$dem1.' lượt bình luận':'Chưa có lượt bình luận nào!'}}</td>
                        </tr>
                ...
                <div class="content">
                    ...
                    <div class="nd">
                        ...
                        <div style="margin: 1em; margin-top: 0; background-color: rgba(196, 195, 194, 0.8); padding: 10px;; border-radius: 10px;">
                            <p><b>{{$ad->name}}</b><span style="background-color: yellow; color: red; padding: 3px; border-radius: 2px; font-weight: bolder;margin-left: 2px;">Admin</span></p>
                            <p>{!!$cm->comment!!}</p>
                        </div>
                    </div>
                    @endif
                    ...
                    @endforeach
                    <div class="comment">
                        <form action="{{route('comment_ad',$forum->id)}}" method="post">
                            @csrf
                            <input type="hidden" name="forum_id" value="{{$forum->id}}">
                            <textarea name="comment" placeholder="Viết bình luận..."></textarea>
                            <small class="error">@error('comment') {{$message}}@enderror</small>
